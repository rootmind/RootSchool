(function () {
	...
	app.controller('dashboardParent1Controller', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'commonControls', 'alertsManager', 'messageFactory', 'appConstants', '$q', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, commonControls, alertsManager, messageFactory, appConstants, $q) {
		...
		$scope.fetchSchoolAttendance = function (duration) {
			...
			methodAction = "fetchStudentAttendanceCount";
			...
			jsonData.returnData(function (value) {
				...
				if (value != null) {
					...
					result = value.fetchStudentAttendanceCount;

					if (value.success == true) {

						if (result.validSession == true) {

							if (result.morningStatusPresentWrapper && result.morningStatusPresentWrapper[0].recordFound == true) {
								schoolMorningAttendanceJSON = result.morningStatusPresentWrapper;
							}
							if (result.eveningStatusPresentWrapper && result.eveningStatusPresentWrapper[0].recordFound == true) {
								schoolEveningAttendanceJSON = result.eveningStatusPresentWrapper;
							...
							deferred.resolve(source1, source2);
