(function () {
    ...
    app.controller('queueSupervisorController', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'messageFactory', 'appConstants', 'firestorageFactory', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$uibModal','$compile', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, messageFactory, appConstants, firestorageFactory, DTOptionsBuilder, DTColumnBuilder, $q, $uibModal, $compile) {
        ...
        dt.columns = [
            DTColumnBuilder.newColumn("code").withTitle("Supervisor Code").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("desc").withTitle("Supervisor Name").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("mobileNo").withTitle("Mobile").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("email").withTitle("email").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("photoID").withTitle("Photo ID").withOption('defaultContent', '-'),
        ...
        function edit(info) {
            ...
            var passInfo = [info.code,info.desc,info.mobileNo, info.email, info.photoID, "UPDATE"];
        ...
        $ctrl.openModal = function (size, parentSelector, passInfo) {
            ...
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'views/transport/supervisor.html',
        ...
        $scope.searchData = function () {
            ...
            //if ($scope.loginForm.wrapper.academicYearID.$valid) {
                ...
                jsonData.returnData(function (value) {
                    ...
                    if (value != null) {
                        ...
                        if (value.success == true) {
                            ...
                            if (result.validSession == true && result.supervisorWrapper[0].recordFound == true) {
                                $scope.supervisorWrapper = result.supervisorWrapper;
                    ...
                    $rootScope.loading = false;
                    deferred.resolve($scope.supervisorWrapper);
                });
            //}
