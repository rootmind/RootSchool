(function () {
	...
	app.controller('queueBookCatalogController', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'commonControls', 'messageFactory', 'appConstants', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$uibModal', '$compile', '$state', '$stateParams', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, commonControls, messageFactory, appConstants, DTOptionsBuilder, DTColumnBuilder, $q, $uibModal, $compile, $state, $stateParams) {
		...
		dt.columns = [
			DTColumnBuilder.newColumn("bookRefNo").withTitle("Ref No").withOption('defaultContent', '-'),
			...
			DTColumnBuilder.newColumn("bookName").withTitle("Book Name").withOption('defaultContent', '-'),
			DTColumnBuilder.newColumn("author").withTitle("Author").withOption('defaultContent', '-'),
			DTColumnBuilder.newColumn("edition").withTitle("Edition").withOption('defaultContent', '-'),
			DTColumnBuilder.newColumn("bookCategoryValue").withTitle("Category").withOption('defaultContent', '-'),
		...
		function edit(info) {
			...
			dt.message = info.bookRefNo + ' - ' + info.bookName;
			var passInfo = [info.bookRefNo, info.bookName, "UPDATE"];
			sharedProperties.setBookRefNo(info.bookRefNo);
		...
		$ctrl.openModal = function (size, parentSelector, passInfo) {
			...
			var modalInstance = $uibModal.open({
				...
				templateUrl: 'views/library/book-catalog.html',
				controller: 'bookCatalogController',
				controllerAs: '$ctrl',
				size: size,
				scope: $scope,
				appendTo: parentElem,
		...
		$scope.searchData = function () {
			...
			console.log('Search fetchBookSearch Data= ' + JSON.stringify(message));
			...
			jsonData.returnData(function (value) {
				...
				console.log('fetchBookSearch Data= ' + JSON.stringify(value));
				if (value != null) {
					...
					if (value.success == true) {
						...
						if (result.validSession == true && result.bookCatalogueWrapper[0].recordFound == true) {
							$scope.wrapper = result.bookCatalogueWrapper;
