(function () {
	...
	app.controller('queueGroupMenuController', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'messageFactory', 'appConstants', 'firestorageFactory', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$uibModal', '$compile', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, messageFactory, appConstants, firestorageFactory, DTOptionsBuilder, DTColumnBuilder, $q, $uibModal, $compile) {
		...
		dt.columns = [
			DTColumnBuilder.newColumn("groupIDValue").withTitle("Group").withOption('defaultContent', '-'),
			...
			DTColumnBuilder.newColumn(null).withTitle('Delete').notSortable().renderWith(actionsHtmlDelete)
		...
		function edit(info) {
			...
			var passInfo = [info.groupID, info.menuID, info.assignFlag, "UPDATE"];
		...
		function actionsHtmlDelete(data, type, full, meta) {
			...
			dt.infos[id_data.id] = data;
			return '<div class="text-center">' +
				'<span class="icon icon-trash text-primary" style="cursor: pointer;" ng-click="dt.remove(dt.infos[' + id_data.id + '])"></span></div>' //<button class="btn btn-outline-primary btn-icon sq-24" ></button>
		}

		function remove(info) {
			// console.log('You are trying to edit the row: ' + JSON.stringify(info));
			//var passInfo = [info.groupID,info.menuID,info.assignFlag];
			...
			Swal.fire({
				...
				if (result.value) {
					$scope.saveData(info.groupID, info.menuID);
				}
			})
		...
		$ctrl.openModal = function (size, parentSelector, passInfo) {
			...
			var modalInstance = $uibModal.open({
				...
				templateUrl: 'views/security/group-menu.html',
				...
				if (value != null) {
					...
					if (value.success == true) {
						...
						if (result.validSession == true && result.groupMenuWrapper[0].recordFound == true) {
							$scope.wrapper = result.groupMenuWrapper;
		...
		$scope.saveData = function (groupID, menuID) {
