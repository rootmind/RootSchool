(function () {
	...
	app.controller('hostelController', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'commonControls', 'alertsManager', 'appConstants', 'messageFactory', '$filter', '$q', '$uibModalInstance', 'passInfo', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, commonControls, alertsManager, appConstants, messageFactory, $filter, $q, $uibModalInstance, passInfo) {
		...
		$ctrl.selected = {
			code: $ctrl.passInfo[0], desc: $ctrl.passInfo[1], totalRooms: $ctrl.passInfo[2], 
			vacantRooms: $ctrl.passInfo[3], supervisorID: $ctrl.passInfo[4], hostelGroup: $ctrl.passInfo[5], 
			mode: $ctrl.passInfo[6] 
		};
		...
		if(sharedProperties.getActionMode()=='UPDATE')
		{
			...
			$scope.wrapper.totalRooms = $ctrl.selected.totalRooms;
			$scope.wrapper.vacantRooms = $ctrl.selected.vacantRooms;
			$scope.wrapper.supervisorID = $ctrl.selected.supervisorID;
			$scope.wrapper.hostelGroup = $ctrl.selected.hostelGroup;
		...
		$scope.saveData = function () {
			...
			if ($scope.loginForm.$valid) {
				...
				methodAction = "updateHostel";
				message = {

					"code": $scope.wrapper.code,
					"desc": $scope.wrapper.desc,
					"totalRooms": $scope.wrapper.totalRooms,
					"vacantRooms": $scope.wrapper.vacantRooms,
					"hostelGroup": $scope.wrapper.hostelGroup,
					"supervisorID": $scope.wrapper.supervisorID
