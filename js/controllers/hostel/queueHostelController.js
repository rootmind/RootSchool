(function () {
    ...
    app.controller('queueHostelController', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'messageFactory', 'appConstants', 'firestorageFactory', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$uibModal','$compile', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, messageFactory, appConstants, firestorageFactory, DTOptionsBuilder, DTColumnBuilder, $q, $uibModal, $compile) {
        ...
        dt.columns = [
            DTColumnBuilder.newColumn("code").withTitle("Hostel ID").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("desc").withTitle("Group").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("totalRooms").withTitle("Total Rooms").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("vacantRooms").withTitle("Vacant Rootms").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("supervisorIDValue").withTitle("Supervisor").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("hostelGroupValue").withTitle("Hostel Group").withOption('defaultContent', '-'),
        ...
        function edit(info) {
            ...
            var passInfo = [info.code,info.desc,info.totalRooms, info.vacantRooms, info.supervisorID, info.hostelGroup, "UPDATE"];
                ...
                jsonData.returnData(function (value) {
                    ...
                    if (value != null) {
                        ...
                        if (value.success == true) {
                            ...
                            if (result.validSession == true && result.hostelWrapper[0].recordFound == true) {
                                $scope.hostelWrapper = result.hostelWrapper;
                    ...
                    $rootScope.loading = false;
                    deferred.resolve($scope.hostelWrapper);
                });
            //}
