(function () {
    ...
    app.controller('queueCalendarActivitiesController', ['$scope', '$rootScope', 'connectHostFactory', '$location', 'sharedProperties', 'messageFactory', 'appConstants', 'firestorageFactory', 'DTOptionsBuilder', 'DTColumnBuilder', '$q', '$uibModal','$compile', function ($scope, $rootScope, connectHostFactory, $location, sharedProperties, messageFactory, appConstants, firestorageFactory, DTOptionsBuilder, DTColumnBuilder, $q, $uibModal, $compile) {
        ...
        dt.columns = [
            DTColumnBuilder.newColumn("calendarRefNo").withTitle("Ref No").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("activityCategoryValue").withTitle("Activity").withOption('defaultContent', '-'),
            DTColumnBuilder.newColumn("startDate").withTitle("Start Date").withOption('defaultContent', '-'),
        ...
        function edit(info) {
            ...
            var passInfo = [info.calendarRefNo, "UPDATE"];
        ...
        $ctrl.openModal = function (size, parentSelector, passInfo) {
            ...
            var modalInstance = $uibModal.open({
                ...
                templateUrl: 'views/admin/calendar-activities.html',
                controller: 'calendarActivitiesController',
                controllerAs: '$ctrl',
                size: size,
                scope: $scope,
                appendTo: parentElem,
        ...
        $scope.searchData = function () {
            ...
            //if ($scope.loginForm.wrapper.academicYearID.$valid) {
                ...
                jsonData.returnData(function (value) {
                    ...
                    $rootScope.loading = false;
                    deferred.resolve($scope.calendarActivitiesWrapper);
                });
            //}
